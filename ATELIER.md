...
Six scripts Python dans `tools/`. Ils tournent sur ta machine, sans internet
...
Et trois vérificateurs dans `_review/` : la planche contact sur fond noir, la
...
6. Lancer node _review/check-images.mjs
...
ce que fait déjà `_backup-*/`.

Et retiens : `_review/stale/` et `_review/unused/` ne sont pas des poubelles.
...
```bash
node _review/check-images.mjs
python _review/contact-sheet.py
```
